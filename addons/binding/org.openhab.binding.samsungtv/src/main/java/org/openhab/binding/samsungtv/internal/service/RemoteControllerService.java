...
public class RemoteControllerService implements SamsungTvService {
	...
	private Logger logger = LoggerFactory
			.getLogger(RemoteControllerService.class);
		...
		case KEY_CODE:
			if (command instanceof StringType) {
				...
				if (key != null) {
					...
					logger.warn("Command '{}' not supported for channel '{}'",
							command, channel);
				}
			}
			break;
	...
	private void sendKeyCode(final KeyCode key) {
		...
		if (host != null) {
			...
			RemoteController remoteController = new RemoteController(host,
					port, "openHAB2", "openHAB2");
		...
		if (host != null) {
			...
			if (remoteController != null) {
				try {
					...
					logger.error(
							"Could not send command(s) to device on {}: {}",
							host + ":" + port, e);
				}
			}
		} else {
