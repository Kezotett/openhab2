...
public class RemoteController {
	...
	private final char[] ACCESS_GRANTED_RESP = new char[] { 0x64, 0x00, 0x01,
			0x00 };

	// User rejected your network remote controller response
	private final char[] ACCESS_DENIED_RESP = new char[] { 0x64, 0x00, 0x00,
			0x00 };

	// waiting for user to grant or deny access response
	private final char[] WAITING_USER_GRANT_RESP = new char[] { 0x0A, 0x00,
			0x02, 0x00, 0x00, 0x00 };
	 ...
	 */
	public RemoteController(String host, int port, String appName,
			String uniqueId) {
		this.host = host;
		this.port = port;
	...
	public void openConnection() throws RemoteControllerException {
		...
		try {
			/*
			 * 
			 * offset value and description ------ --------------------- 0x00
			 * 0x00 - datagram type? 0x01 0x0013 - string length (little endian)
			 * 0x03 "iphone.iapp.samsung" - string content 0x16 0x0038 - payload
			 * size (little endian) 0x18 payload
			 * 
			...
			try {
				 ...
				 * 0x00 don't know, it it always 0x00 or 0x02 0x01 0x000c -
				 * string length (little endian) 0x03 "iapp.samsung" - string
				 * content 0x0f 0x0006 - payload size (little endian) 0x11
				 * payload
				 */
				...
				if (Arrays.equals(result, ACCESS_GRANTED_RESP)) {
					...
					throw new RemoteControllerException(
					...
					throw new RemoteControllerException(
							"Unknown response received for access query");
				}
	...
	private void sendKeyData(KeyCode key) throws RemoteControllerException {
		...
		/*
		 * offset value and description ------ --------------------- 0x00 always
		 * 0x00 0x01 0x0013 - string length (little endian) 0x03
		 * "iphone.iapp.samsung" - string content 0x16 0x0011 - payload size
		 * (little endian) 0x18 payload
		 */
		try {
			writer.append((char) 0x00);
			writeString(writer, APP_STRING);
	...
	private String createKeyDataPayload(KeyCode key) throws IOException {
		 ...
		 * Payload:
		 * 
		 * offset value and description ------ --------------------- 0x18 three
		 * 0x00 bytes 0x1b 0x000c - key code size (little endian) 0x1d key code
